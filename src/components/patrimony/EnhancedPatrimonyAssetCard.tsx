...
import { Asset, Investment, Debt, ASSET_TYPE_LABELS, LIQUIDITY_LABELS, RISK_LABELS, CATEGORY_COLORS } from '@/types/patrimony';
...
export const EnhancedPatrimonyAssetCard: React.FC<EnhancedPatrimonyAssetCardProps> = ({
  ...
  // Get the primary insight for the mini-insight bar
  const primaryInsight = assetInsights.find(i => 
    i.severity === 'warning' || i.severity === 'critical'
  ) || assetInsights.find(i => i.severity === 'success');
  ...
  return (
    <Card className="bg-zinc-900/80 border-zinc-800 overflow-hidden">
      ...
      <div className="p-4">
        ...
        {primaryInsight && (
          <div className="mt-3">
            <AssetInsightChip insight={primaryInsight} compact />
          </div>
        )}
