            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {availableIcons.map((iconOpt) => {
                  ...
                  return (
                   Voy a completar el archivo con el componente InvestmentForm incluido:

<dyad-write path="src/pages/dashboard/InvestmentsPage.tsx" description="Adding edit and delete functionality to InvestmentsPage with InvestmentForm component">
"use client";
...
const InvestmentForm = ({ investment, setInvestment, onSubmit, saving, error, setError, loans, patrimony, isLoanDialogOpen, setIsLoanDialogOpen, isPatrimonyDialogOpen, setIsPatrimonyDialogOpen, isNewPatrimonyOpen, setIsNewPatrimonyOpen, newPatrimonyAsset, setNewPatrimonyAsset, handleCreatePatrimony, isCategoryDialogOpen, setIsCategoryDialogOpen, isCreateCategoryDialogOpen, setIsCreateCategoryDialogOpen, getSelectedCategoryInfo, customCategories, availableIcons, categoryColors, newCustomCategory, setNewCustomCategory, handleCreateCustomCategory, getLoanLabel, getPatrimonyLabel, isNew }: any) => {
  const getLoanLabelLocal = () => investment.loan_id === "none" ? "Vincular a préstamo" : loans.find((l: any) => l.id === investment.loan_id)?.name || "Vincular a préstamo";
  const getPatrimonyLabelLocal = () => investment.patrimony_id === "none" ? "Vincular a patrimonio" : patrimony.find((p: any) => p.id === investment.patrimony_id)?.name || "Vincular a patrimonio";
