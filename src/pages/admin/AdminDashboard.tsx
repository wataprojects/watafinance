...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("admin-get-dashboard-data");

    if (error || data?.error) {
      console.error("[AdminDashboard] Error fetching dashboard data:", error || data.error);
      setIsLoading(false);
      return;
    }
  ...
  const incomeExpenseTotal = metrics.totalIncomes + metrics.totalExpenses;
  const incomePercentage = incomeExpenseTotal > 0 ? (metrics.totalIncomes / incomeExpenseTotal) * 100 : 0;
  const expensePercentage = incomeExpenseTotal > 0 ? (metrics.totalExpenses / incomeExpenseTotal) * 100 : 0;

  return (
    <div className="min-h-screen bg-slate-950 flex">
      <AdminSidebar />
      <div className="flex-1 flex flex-col">
        <AdminHeader title="Dashboard" subtitle="Resumen general de Monyro" />
                    ...
                    <CardContent className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <div className="h-2 bg-slate-800 rounded-full overflow-hidden flex">
                          ...
                          <div className="h-full bg-red-500" style={{ width: `${expensePercentage}%` }} />
                        </div>
                      </div>
